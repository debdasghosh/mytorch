    ...
    if grad_fn is not None:
        ...
        for i in range(len(grad_fn.next_functions)):
            if isinstance(grad, tuple):
                ...
                # if grad_fn.next_functions[i].variable.grad is None:
                #     grad_fn.next_functions[i].variable.grad = this_grad
                # else:
                #   grad_fn.next_functions[i].variable.grad += this_grad
            # elif isinstance(grad_fn.next_functions[i], BackwardFunction):
            #     backward(grad_fn.next_functions[i], this_grad)
...
class Function:
    ...
    def apply(cls, *args):
        ...
        for arg in args:
            if hasattr(arg, 'is_leaf') and hasattr(arg, 'requires_grad'):
                if arg.is_leaf and arg.requires_grad:
                    accGrad = AccumulateGrad(arg)
                    if arg.grad_fn is None:
                        arg.grad_fn = accGrad
                    backward_function.next_functions.append(accGrad)
                elif arg.is_leaf and not arg.requires_grad: 
                    backward_function.next_functions.append(None)
                elif not arg.is_leaf and arg.requires_grad:
                    backward_function.next_functions.append(arg.grad_fn)
                elif not arg.is_leaf and not arg.requires_grad:
