...
class Tensor:
    ...
    def __init__(self, data, requires_grad=False, is_leaf=True,
        ...
        self.grad_fn = None # Set during forward pass : Node
    ...
    def backward(self):
        ...
        #self.grad = Tensor(np.ones(self.data.shape), requires_grad=True, is_leaf=False)
        return autograd_engine.backward(self.grad_fn, Tensor(np.ones(self.data.shape), requires_grad=True, is_leaf=False) )
    ...
    def unsqueeze(self,dim=0):
        ...
        mod_shape = np.insert(curr_shape, dim, 1, axis = 0)
        return self.reshape((*mod_shape))


    def __pow__(self, other):
        return F.Power.apply(self, Tensor(other))
